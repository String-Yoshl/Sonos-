...
1. 常時起動するホストでサーバを起動: `python -m sonos_sleep_bgm.cli serve`
   （起動時にスマホ用 URL `http://<PCのIP>:8765/?token=...` が表示されます）
...
- 🔌 **SwitchBot 連携（任意）** … セットごとに SwitchBot デバイスの ON/OFF を紐づけ可能（例: 21:00 に BGM 開始と同時に**エアコンを消す**、スリープタイマー終了時に消す）。⚙ 設定で SwitchBot アプリの「開発者向けオプション」のトークン/シークレットを登録して使う。
...
- **API はトークン認証必須**です。初回起動時にランダムなトークンが生成され
  `data/auth_token`（所有者のみ読み取り可）に保存・再利用されます。
- 起動時に表示される `?token=...` 付き URL で一度開けば、トークンは端末の
  localStorage に保存され、以後は URL なしで使えます（変更するまで有効）。
...
- `data/`（保存した設定・トークン）は `.gitignore` 済みでコミットされません。
